    ...
    for event in pygame.event.get():  # Барлық оқиғаларды тексеру
        if event.type == pygame.QUIT:  # Егер терезені жабу оқиғасы болса
            ...
            if event.key == pygame.K_c:
                ...
                mode = "r_triangle"  # Тік бұрышты үшбұрыш салу режимі
            elif event.key == pygame.K_q:
                mode = "eq_triangle"  # Теңқабырғалы үшбұрыш салу режимі
            elif event.key == pygame.K_h:
                mode = "rhombus"  # Ромб салу режимі
            ...
            if mode == "rect":  # Егер режим тіктөртбұрыш болса
                ...
                pygame.draw.polygon(screen, color, [start_pos, (event.pos[0], start_pos[1]), start_pos, event.pos], 2)
            elif mode == "eq_triangle":
                height = (3 ** 0.5 / 2) * (event.pos[0] - start_pos[0])
                pygame.draw.polygon(screen, color, [start_pos, (event.pos[0], start_pos[1]),
                                                    ((start_pos[0] + event.pos[0]) // 2, start_pos[1] - height)], 2)
            elif mode == "rhombus":
                dx = (event.pos[0] - start_pos[0]) // 2
                dy = (event.pos[1] - start_pos[1]) // 2
                pygame.draw.polygon(screen, color,
                                    [(start_pos[0], start_pos[1] - dy), (start_pos[0] + dx, start_pos[1]),
                                     (start_pos[0], start_pos[1] + dy), (start_pos[0] - dx, start_pos[1])], 2)
